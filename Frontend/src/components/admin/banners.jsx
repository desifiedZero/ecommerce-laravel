...
export default function AdminBanners(props) {
    ...
    const fetchBanners = () => {
        fetch(process.env.REACT_APP_BASE_URL.concat(ApiRoutes.admin.banners.getAll), {
            headers: {
    ...
    useEffect(() => {
        setButtons(banners.map(x => 
            ...
            >
                <ListItemText primary={x?.tag} secondary={"ID: " + x?.id} />
            </ListItemButton>));
    }, [banners, selectedBanner, disabled]);

    const noUser = <ListItemButton>
        <ListItemText primary={"No banners available"} />
    </ListItemButton>;
                        ...
                        <Grid container sx={{ width: '100%', bgcolor: 'background.paper' }}>
                            <Grid item sm={12} style={{marginBottom: '1rem'}}>
                                <Button variant="contained" color="success" size="large" onClick={() => {
                                    ...
                                    setNewItem(true);
                                    setViewItem(false);
                                }} disabled={disabled || newItem}>New Banner</Button>
                            </Grid>
                            <Grid item sm={12}>
                                {newItem && <CreateBannerInfo user={selectedBanner} newItem={newItem} setNewItem={setNewItem} disableChange={disableChange} updated={onUpdated} type={"admin"}/>}
                                {viewItem && <ViewBannerInfo user={selectedBanner} updated={onUpdated}/>}
                            </Grid>
                        </Grid>
                    </Grid>
                </Grid>
