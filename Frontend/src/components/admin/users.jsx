...
export default function AdminAdmin(props) {
    ...
    const fetchUsers = () => {
        fetch(process.env.REACT_APP_BASE_URL.concat(ApiRoutes.admin.user.getAll), {
            headers: {
            ...
            <Container>
                ...
                <Grid container sx={{ width: '100%', bgcolor: 'background.paper' }}>
                    ...
                    <Grid item xs={12} sm={8} style={{paddingLeft: '1rem'}}>
                        <DisplayUserInfo user={selectedUser} newItem={newItem} setNewItem={setNewItem} disableChange={disableChange} updated={onUpdated} type={"user"}/>
                    </Grid>
                </Grid>
            </Container>
        </div>;
